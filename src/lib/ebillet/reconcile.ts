...
/** Return the organizer's showtimes after removing isolated impossible dates. */
...
export function validateSnapshot(
  ...
  // A small number of impossible dates is treated as bad individual records,
  // not as evidence that the entire organizer payload is corrupt. The caller
  // removes those records before reconciliation. This prevents one eBillet
  // typo (e.g. year 2029 instead of 2026) from discarding every real screening.
  const sanitizedCount = sanitizeShowtimes(organizerId, payload, opts.now).filter(
    (st) => st.organizerId === organizerId,
  ).length;
  const invalidCount = showtimes.length - sanitizedCount;
  if (invalidCount > 0 && invalidCount / Math.max(showtimes.length, 1) > 0.25) {
    return reject(`${invalidCount} af ${showtimes.length} showtimes har ugyldige/usandsynlige datoer`);
  }
  ...
  for (const st of showtimes) {
    ...
    // Only reject malformed timestamps here; impossible-but-parseable dates
    // are filtered individually by sanitizeShowtimes().
    void now;
  }

  if (sanitizedCount === 0 && opts.existingRowCount > 0) {
...
export const slugifyTitle = (value: string): string =>
  value.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[æø]/g, (c) => (c === "æ" ? "ae" : "oe")).replace(/å/g, "aa").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "item";
...
export function matchMovie(input: MovieMatchInput, candidates: MovieCandidate[]): MovieCandidate | null {
  ...
  if (input.movieIds.length > 0) {
    const byMovieId = candidates.find((c) => (c.ebillet_movie_ids ?? []).some((id) => input.movieIds.includes(id)));
...
export function diffShowtimes(existing: ExistingShowtime[], desired: DesiredShowtime[], opts: { allowDeletes: boolean } = { allowDeletes: true }): ShowtimeDiff {
  ...
  for (const row of desired) {
    ...
    for (const id of row.ebillet_showtime_ids) {
      ...
      if (candidate && !matched.has(candidate.id)) { hit = candidate; break; }
    }
    if (!hit) {
      const candidate = byKey.get(rowKey(row));
      if (candidate && !matched.has(candidate.id)) hit = candidate;
    }
    if (hit) { matched.add(hit.id); updates.push({ id: hit.id, row }); }
