...
function eBilletMovieKey(showtime) {
  const baseId = Number(showtime.movieBaseId ?? 0);
  return baseId > 0 ? `base-${baseId}` : `movie-${showtime.movieId}`;
}

function canonicalMovieKeys(movie) {
  const keys = new Set();
  if (Number(movie?.ebillet_movie_base_id ?? 0) > 0) {
    keys.add(`base-${Number(movie.ebillet_movie_base_id)}`);
  }
...
async function auditEmpire(config, start, end) {
  let cinemas = await rest({
    ...config,
    table: "cinemas",
    params: {
      select: "id,slug,name,city,ebillet_organizer_id",
      slug: "eq.empire-bio",
      limit: "5",
    },
  });
  ...
  const screenings = await paged({
    ...config,
    table: "screenings",
    params: {
      select:
        "id,source,source_ref,movie_id,starts_at,local_date,local_time,hall,ticket_url,updated_at",
      cinema_id: `eq.${cinema.id}`,
      source: "eq.ebillet",
      local_date: [`gte.${start}`, `lte.${end}`],
      order: "starts_at.asc",
    },
  });
  ...
  const movies = movieIds.length
    ? await rest({
        ...
        table: "movies",
        params: {
          select: "id,title,slug,ebillet_movie_base_id,ebillet_movie_ids,tmdb_id",
          id: `in.(${movieIds.join(",")})`,
        },
      })
    : [];
  ...
  const upstreamResponse = await fetch(apiUrl, {
    headers: { "User-Agent": "KarlVictor", accept: "application/json" },
  });
  if (!upstreamResponse.ok) {
    throw new Error(`Empire eBillet upstream failed (${upstreamResponse.status})`);
  }
  const upstream = await upstreamResponse.json();
  const upstreamRows = (upstream.showtimes ?? []).filter(
    (row) =>
      Number(row.organizerId) === Number(cinema.ebillet_organizer_id) &&
      clean(row.dateTime).slice(0, 10) >= start &&
      clean(row.dateTime).slice(0, 10) <= end,
  );
  ...
  for (const screening of screenings) {
    ...
    if (!keys.some((key) => upstreamKeys.has(key))) {
      staleCanonical.push({
        id: screening.id,
        ...
        time: time5(screening.local_time),
        hall: screening.hall,
  ...
  const missingCanonical = upstreamRows
    .filter((row) => !canonicalKeys.has(upstreamPhysicalKey(row)))
    .map((row) => ({
      id: row.id,
      movieId: row.movieId,
      movieBaseId: row.movieBaseId,
      dateTime: row.dateTime,
      hall: row.locationName,
      buyEnabled: row.buyInfo?.enabled ?? null,
      key: upstreamPhysicalKey(row),
    }));

  return {
    cinema,
    canonicalCount: screenings.length,
    upstreamCount: upstreamRows.length,
    staleCanonicalCount: staleCanonical.length,
    staleCanonical: staleCanonical.slice(0, 100),
...
async function auditOdyssey(config, start, end) {
  const movies = await rest({
    ...config,
    table: "movies",
    params: {
      select:
        "id,slug,title,original_title,source,year,release_date,external_id,ebillet_movie_base_id,ebillet_movie_ids,tmdb_id,tmdb_status,created_at",
      title: "ilike.*Odyssey*",
      order: "title.asc",
      limit: "100",
    },
  ...
  const screenings = await paged({
    ...
    table: "screenings",
    params: {
      select: "movie_id,source,cinema_id,local_date,local_time,hall,events,formats",
  ...
  const enriched = movies.map((movie) => {
    ...
    return {
      ...movie,
      normalizedTitle: normalizeFranchiseTitle(movie.title),
      screeningCount: stat?.screenings ?? 0,
      cinemaCount: stat?.cinemas.size ?? 0,
      screeningSources: [...(stat?.sources ?? [])],
      eventLabels: [...(stat?.eventLabels ?? [])],
      formats: [...(stat?.formats ?? [])],
      firstDate: stat?.firstDate ?? null,
  ...
  return {
    movies: enriched,
    groups: [...grouped.entries()].map(([key, rows]) => ({
      key,
      count: rows.length,
      ids: rows.map((row) => row.id),
      titles: rows.map((row) => row.title),
      tmdbIds: [...new Set(rows.map((row) => row.tmdb_id).filter(Boolean))],
      screeningCount: rows.reduce((sum, row) => sum + row.screeningCount, 0),
    })),
  };
}
...
const [empire, odyssey] = await Promise.all([
  auditEmpire(config, start, end),
  auditOdyssey(config, start, end),
]);
