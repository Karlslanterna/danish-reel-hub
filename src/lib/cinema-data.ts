...
/**
 * Compatibility shape consumed by the existing UI. The database source of
 * truth is `screenings` (one row per physical screening); rows are grouped into
 * this legacy date/hall/times shape only at the read boundary.
 */
...
/**
 * TMDb is preferred for film metadata; source metadata is the fallback. The
 * coalesce is per field, so a partial TMDb record never blanks a source field.
 */
...
type ShowtimeGroup = Showtime & {
  slots: Array<{ time: string; startsAt: string; ticketUrl: string | null }>;
};

/** Convert canonical screening rows into the unchanged UI showtime contract. */
export function groupScreeningsForUi(rows: ScreeningRow[]): Showtime[] {
...
/**
 * Movies ordered by a named strategy. Ordering happens in `movies_ranked`,
 * which is backed by canonical `screenings`.
 */
...
export async function fetchShowtimesForMovie(movieId: string): Promise<Showtime[]> {
  ...
  if (error) throw error;
  return groupScreeningsForUi((data ?? []) as ScreeningRow[]);
}
...
/** Lightweight index used by the homepage for radius + date filtering. */
export async function fetchShowtimeIndex(): Promise<ShowtimeIndexRow[]> {
  ...
  for (const row of data ?? []) {
    ...
    addUnique(existing.formats, row.formats);
    addUnique(existing.languages, row.languages);
    addUnique(existing.events, row.events);
...
export async function fetchMoviesAndShowtimesForCinemas(
  ...
  const rows = (data ?? []) as Array<ScreeningRow & { movies: MovieRow | null }>;
  const seen = new Set<string>();
  const movies: Movie[] = [];
  for (const row of rows) {
