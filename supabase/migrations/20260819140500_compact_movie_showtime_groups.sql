create or replace function public.get_movie_showtime_groups(
  p_movie_ids text[],
...
as $$
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'hall', grouped.hall,
        'times', grouped.times,
        'ticket_urls', grouped.ticket_urls,
  ...
  from (
    select
      ...
      array_agg(
        coalesce(screenings.ticket_url, '')
        order by screenings.starts_at, screenings.id
      ) as ticket_urls
...
revoke all on function public.get_movie_showtime_groups(text[], timestamptz, date, date)
from public;

grant execute on function public.get_movie_showtime_groups(text[], timestamptz, date, date)
