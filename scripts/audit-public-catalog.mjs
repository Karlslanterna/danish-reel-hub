...
const normalizePresentationTitle = (value) =>
  normalizeTitle(value)
    .replace(/\b(2d|3d|4dx|imax|org tale|original tale|dansk tale|tekstet|dk tale|eng tale)\b/g, " ")
    .replace(/\s+/g, " ")
    .trim();

const GENERIC_EVENT_TITLE = /^(særvisning|saervisning|børnebiffen|bornebiffen|babybio|seniorbio|strikkebio|formiddagsbio|filmklub|specialvisning|event)(\b|\s*[-:])/i;
const NON_FILM_TITLE = /^(opera|ballet|teater|koncert|stand[ -]?up|live event|foredrag)(\b|\s*[-:])/i;
...
const [movies, screenings] = await Promise.all([
  fetchPages({
    ...
    key,
    table: "movies_ranked",
    select: "id,title,original_title,year,runtime,genre,slug,external_id,tmdb_id,tmdb_status,tmdb_skip_reason,poster,tmdb_poster_url,screening_count,next_screening_date",
  ...
  fetchPages({
    ...
    key,
    table: "screenings",
    select: "movie_id,cinema_id,source,starts_at,local_date,local_time,formats,languages,events,ticket_url",
    configure: (requestUrl) => {
...
for (const row of screenings) screeningCountByMovie.set(row.movie_id, (screeningCountByMovie.get(row.movie_id) ?? 0) + 1);
...
console.log(JSON.stringify({
  ...
  sources: Object.fromEntries([...groupBy(screenings, (row) => row.source).entries()].map(([source, rows]) => [source, rows.length])),
}, null, 2));

const genericTitles = activeMovies.filter((movie) => GENERIC_EVENT_TITLE.test(String(movie.title ?? "").trim()));
const nonFilmTitles = activeMovies.filter((movie) => NON_FILM_TITLE.test(String(movie.title ?? "").trim()));
...
printRows("Generic/event-shell titles", genericTitles, ["id", "title", "year", "runtime", "genre", "tmdb_id", "tmdb_status", "external_id", "screening_count"]);
printRows("Likely non-film titles", nonFilmTitles, ["id", "title", "year", "runtime", "genre", "tmdb_id", "external_id", "screening_count"]);
printRows("Generic titles incorrectly enriched by TMDb candidate", genericWithTmdb, ["id", "title", "tmdb_id", "tmdb_status", "tmdb_poster_url", "screening_count"]);
printRows("Very weak active movie records", weakMovies, ["id", "title", "external_id", "tmdb_status", "tmdb_skip_reason", "screening_count"]);

const exactDuplicateGroups = [...groupBy(activeMovies, (movie) => normalizeTitle(movie.title)).entries()]
...
for (const [title, group] of exactDuplicateGroups.slice(0, 80)) {
  console.log(JSON.stringify({ title, movies: group.map((movie) => ({ id: movie.id, title: movie.title, year: movie.year, tmdb_id: movie.tmdb_id, external_id: movie.external_id, screenings: screeningCountByMovie.get(movie.id) ?? 0 })) }));
}

const presentationDuplicateGroups = [...groupBy(activeMovies, (movie) => normalizePresentationTitle(movie.title)).entries()]
  .filter(([, group]) => group.length > 1)
  .sort((a, b) => b[1].length - a[1].length || a[0].localeCompare(b[0]));
...
const posterCollisions = [...groupBy(activeMovies, posterUrl).entries()]
  .map(([poster, group]) => ({ poster, group, titles: new Set(group.map((movie) => normalizeTitle(movie.title))) }))
...
for (const collision of posterCollisions.slice(0, 50)) {
  console.log(JSON.stringify({ poster: collision.poster, movies: collision.group.map((movie) => ({ id: movie.id, title: movie.title, tmdb_id: movie.tmdb_id })) }));
}
...
const malformedScreenings = screenings.filter((row) =>
  !/^\d{4}-\d{2}-\d{2}$/.test(String(row.local_date ?? "")) ||
  !/^\d{2}:\d{2}/.test(String(row.local_time ?? "")) ||
  !row.movie_id || !row.cinema_id,
);
printRows("Malformed screening rows", malformedScreenings, ["movie_id", "cinema_id", "source", "starts_at", "local_date", "local_time"]);
